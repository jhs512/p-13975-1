...
class HomeController {
    ...
    fun main(): String {
        val host = runCatching { InetAddress.getLocalHost() }
            .getOrElse { InetAddress.getLoopbackAddress() }

        return """
            |<h1>API 서버</h1>
            |<p>Host Name: ${host.hostName}</p>
            |<p>Host Address: ${host.hostAddress}</p>
            |<div>
    ...
    fun session(session: HttpSession): Map<String, Any?> {
        val names = session.attributeNames.toList()

        return names.associateWith(session::getAttribute)
    }
}